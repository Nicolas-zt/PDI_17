...
foreach my $file (@files) {
    ...
    foreach my $line (@file_content) {
        ...
        if ($line =~ /PROC:/) {
            ...
            $node_lat = $line =~ /NODE_LATITUDE:/ ? 0 + extract_value($line) : undef;  # Extract latitude
        } elsif ($line =~ /NODE_LONGITUDE:/) {
            $node_lon = $line =~ /NODE_LONGITUDE:/ ? 0 + extract_value($line) : undef;  # Extract longitude
        } elsif ($line =~ /NODE_ELEVATION:/) {
            $node_elevation = $line =~ /NODE_ELEVATION:/ ? 0 + extract_value($line) : undef;  # Extract elevation
        } elsif ($line =~ /NODE_URL:/) {
